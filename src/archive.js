(function attachDepotArchive(root){
  ...
  function setTabs(){
    ...
    byId('archiveBreadcrumb').textContent=`Archive / ${onOffices?'Verified Sections & Branches':'Verified Documents'}${detail?` / ${detail}`:''}`;
  }
  function exclusionNotice(){
    return `<aside class="archive-verification"><b>Verification boundary</b><ul>${verified.excluded[procedure].map(item=>`<li>${esc(item)}</li>`).join('')}</ul></aside>`;
  }
  function render(){
    setTabs();
    ...
    if(section==='offices'){
      ...
      content.innerHTML=`${exclusionNotice()}<div class="archive-grid">${items.map(([id,profile],index)=>`<button class="archive-card" data-office="${esc(id)}" type="button">
      ...
      content.innerHTML=`${exclusionNotice()}<div class="archive-grid document-grid">${items.map((profile,index)=>`<button class="archive-card document-card" data-document="${esc(profile.id)}" type="button">
        ...
        <em>${profile.flowchart.length} verified/qualified stages</em></button>`).join('')}</div>`;
    }
  }
  function renderOffice(id){
    const profile=officeProfiles()[id],docs=documentsAtOffice(id),content=byId('archiveContent');
    if(!profile)throw new Error(`No verified Archive profile for ${id}.`);
    content.innerHTML=`${backButton()}<article class="archive-dossier verified-dossier">
      ...
      <div class="source-verdict verified">SOURCE-VERIFIED OFFICE PROFILE</div>
      ...
      <section><h4>Verified documents that pass through this point</h4><div class="office-document-flow">${docs.map(doc=>`<button type="button" data-jump-document="${esc(doc.id)}"><b>${esc(doc.title)}</b><span>${esc(doc.flowchart.find(stage=>stage.officeId===id)?.text||'Verified stage')}</span><small>Open lifecycle flowchart</small></button>`).join('')||'<p>No separately indexed source document passes through this supporting point.</p>'}</div></section>
  ...
  function renderFlowchart(profile){
    return `<div class="document-flowchart" role="list" aria-label="${esc(profile.title)} lifecycle">${profile.flowchart.map((stage,index)=>`<div class="flow-step ${esc(stage.status)}" role="listitem">
      ...
      <div class="flow-node-body"><span>${esc(stage.status==='conflict'?'SOURCE CONFLICT':stage.status==='conditional'?'CONDITIONAL STAGE':'VERIFIED STAGE')}</span><h5>${esc(stage.label)}</h5><p>${esc(stage.text)}</p><small>${citation(stage.evidence)}</small></div>
  ...
  function renderDocument(id){
    ...
    if(!profile)throw new Error(`No verified document profile for ${id}.`);
    ...
    content.innerHTML=`${backButton()}<article class="archive-dossier document-dossier verified-dossier">
      <header><div class="dossier-icon">${iconForDocument(id)}</div><div><span>${procedure.toUpperCase()} · VERIFIED DOCUMENT</span><h3>${esc(profile.title)}</h3><p>${esc(profile.form)}</p></div></header>
      <div class="source-verdict verified">ONLY SOURCE-SUPPORTED CONTENT IS SHOWN</div>
      ...
      <section><h4>Verified contents and endorsements</h4><div class="verified-fields">${profile.contents.map(item=>`<div class="verified-field"><span>${esc(item.side==='reverse'?'REVERSE':'FORM / RECORD')}</span><b>${esc(item.name)}</b><p>${esc(item.text)}</p><small>${citation(item.evidence)}</small></div>`).join('')}</div></section>
  ...
  function openDetail(id){detail=id;try{render()}catch(error){byId('archiveContent').innerHTML=`${backButton()}<div class="feedback">Verified Archive page could not be rendered: ${esc(error.message)}</div>`;bindBack();console.error(error)}}
  function open(){
  ...
  function initialize(){
    ...
    byId('archiveOfficesTab').textContent='Verified Sections / Branches';
    byId('archiveDocumentsTab').textContent='Verified Documents';
