(function attachSmokeRunner(root){
  ...
  async function completeCurrentScenario(label,exerciseRecall=false){
    ...
    while(game.running&&guard++<200){
      ...
      if(!clickCorrect(game))throw new Error(`${label}: no supported correct control at ${target}: ${document.getElementById('miniModal').innerText.slice(0,160)}`);
  ...
  async function officeIntelClickProbe(){
    ...
    canvas.dispatchEvent(new MouseEvent('click',{bubbles:true,clientX:bounds.left+(office.x+office.w/2)*bounds.width/root.DepotMap.width,clientY:bounds.top+(office.y+office.h/2)*bounds.height/root.DepotMap.height}));
  ...
  async function runMatrix(){
    ...
    try{
      await officeIntelClickProbe();await wrongOfficeProbe();
    ...
    const positionVariety=new Set(correctChoicePositions).size;if(positionVariety<2)failures.push({label:'answer randomisation',error:'Correct choices did not move between positions'});const totals=results.reduce((sum,item)=>sum+item.steps,0),summary={status:failures.length?'FAIL':'PASS',scenarios:specs.length,passed:results.length,failed:failures.length,controlPointsExercised:totals,routeRecallChallenges:results.filter(item=>item.recall).length,evidenceReviewLabs:reviewLabs.length,correctAnswerPositions:[...new Set(correctChoicePositions)].sort(),officeIntelClickProbe:true,wrongOfficeExplanationProbe:true,procedures:[...new Set(results.map(item=>item.procedure))],modes:[...new Set(results.map(item=>item.mode))],failures};
