(function bootstrap(root){
  ...
  map.canvas.addEventListener('pointerup',event=>{if(!engine.game.running||!document.getElementById('miniOverlay').classList.contains('hidden'))return;const office=map.buildingAt(canvasPoint(event),engine.game.procedure);if(!office)return;ui.showOfficeIntel(office.id,engine.game.procedure);if(office.id===engine.game.route[engine.game.index])engine.navigateToOffice(office.id)});
